...
app.post('/complete-tournament/:tournamentId', async (req, res) => {
    try {
        ...
        await NotificationService.sendTournamentNotification(tournamentId, `Tournament ${tournament.title} Completed`, 'Thank you for participating! Results will be announced soon.', 'normal');
        res.status(200).json({
            success: true,
...
app.post('/check-tournament-statuses', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to check tournament statuses'
        });
    }
});
...
app.post('/send-tournament-notification/:tournamentId', async (req, res) => {
    try {
        ...
        logger.error('Error sending tournament notification:', error);
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to send tournament notification'
        });
    }
});
...
app.get('/tournament-status/:tournamentId', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to get tournament status'
        });
    }
});
        ...
        if (rule.adminOverride && rule.overrideDistribution) {
            ...
            if (rule.squadSplit) {
                ...
                distribution = [
                    ...
                    { position: 2, percentage: Math.round((rest / pool) * 100 / 2), amount: Math.round(rest / 2) },
                    { position: 3, percentage: Math.round((rest / pool) * 100 / 2), amount: Math.floor(rest / 2) }
                ];
            }
            else {
                // All to 1st
            ...
            if (winners.length > 0 && d.amount && d.amount > 0) {
                ...
                for (const winner of winners) {
                    try {
                        ...
                        await storage.creditUserWallet(userId, amountPerWinner, currency, `Tournament prize for position ${d.position} in ${tournamentTitle}`);
                        // Mark result as paid
