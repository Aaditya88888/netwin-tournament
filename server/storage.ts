/*
 * ADMIN WALLET MANAGEMENT - SINGLE SOURCE OF TRUTH
 *
 * This file handles wallet balance updates through admin approval:
 * 1. Admin approves deposit: Adds amount to user.walletBalance
 * 2. Admin approves withdrawal: Subtracts amount from user.walletBalance
 * 3. Creates transaction record with processed: true
 * 4. Updates existing pending transaction to APPROVED/processed
 *
 * The user.walletBalance field is the ONLY source of truth for user balance.
 */
...
interface Tournament {
    ...
    description?: string;
    gameType?: string;
    game?: string; // Added for game field
    matchType?: string;
    format?: string; // Added for format field
    map?: string;
    startTime?: Date;
    startDate?: string; // Added for startDate field
    endDate?: string; // Added for endDate field
    registrationDeadline?: string; // Added for registration deadline
    entryFee?: number;
    prizePool?: number;
    maxTeams?: number;
    maxParticipants?: number; // Added for maxParticipants field
    registeredTeams?: number;
    currentParticipants?: number; // Added for currentParticipants field
    status?: string;
    location?: string; // Added for location field
    isOnline?: boolean; // Added for online/offline indicator
    organizer?: string; // Added for organizer field
    createdAt?: Date;
    updatedAt?: Date; // Added for updatedAt field
    prizeDistributionRule?: {
        firstPlacePercent: number;
        squadSplit: boolean;
        adminOverride?: boolean;
        overrideDistribution?: any;
    };
}
...
export class FirestoreStorage {
  ...
  async createUser(user: Partial<User>): Promise<User> {
    ...
    return {id: newUser.id, ...newUser.data()} as User; // Keep as string ID
  }
  ...
  async deleteUser(id: string | number): Promise<boolean> {
    try {
      ...
      let uidToDelete: string | undefined = undefined;

      // First, try to find the user document by document ID (most common case)
      const docToDelete = await db.collection("users").doc(idStr).get();
  ...
  async deleteUserData(id: string | number): Promise<void> {
    ...
    transactionsRef.docs.forEach((doc: QueryDocumentSnapshot<DocumentData>) => batch.delete(doc.ref));
    ...
    registrationsRef.docs.forEach((doc: QueryDocumentSnapshot<DocumentData>) => batch.delete(doc.ref));
    // Delete user match history
    const matchesRef = await db.collection("results").where("userId", "==", userId).get();
    matchesRef.docs.forEach((doc: QueryDocumentSnapshot<DocumentData>) => batch.delete(doc.ref));
    // Delete user wallet
  ...
  async getAllUsers(): Promise<User[]> {
    try {
      ...
      const firestoreUsers = snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => {
        const data = doc.data();
        return {
      ...
      if (auth) {
        try {
          ...
          authUsers = listUsersResult.users.map((userRecord: UserRecord) => {
      ...
      firestoreUsers.forEach((user: User) => {
        if (user.uid) firestoreUidMap.set(user.uid, user);
      });
      ...
      try {
        ...
        kycSnapshot.docs.forEach((doc: QueryDocumentSnapshot<DocumentData>) => {
    ...
    try {
      ...
      if (users.length > 0) {
        try {
          ...
          const userIds = users.map((user: User) => user.uid || user.id);
  ...
  async createTournament(tournament: Partial<Tournament>): Promise<Tournament> {
    ...
    return {id: newTournament.id, ...newTournament.data()} as Tournament;
  }
  async updateTournament(id: string, tournamentData: Partial<Tournament>): Promise<Tournament | undefined> {
    const docRef = db.collection("tournaments").doc(id);
    await docRef.update(tournamentData);
    const updated = await docRef.get();
    return updated.exists ? {id, ...updated.data()} as Tournament : undefined;
  }
  ...
  async getAllTournaments(): Promise<Tournament[]> {
    try {
      ...
      const tournaments = snapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
      } as Tournament));
  ...
  async getUpcomingTournaments(): Promise<Tournament[]> {
    try {
      const snapshot = await db.collection("tournaments")
        ...
        .get();
      return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: doc.id, ...doc.data()} as Tournament));
    } catch (error) {
  ...
  async getRegistration(id: string | number): Promise<Registration | undefined> {
    const doc = await db.collection("tournament_registrations").doc(id.toString()).get();
    return doc.exists ? {id: doc.id, ...doc.data()} as Registration : undefined;
  }
  async createRegistration(registration: Partial<Registration>): Promise<Registration> {
    ...
    return {id: newRegistration.id, ...newRegistration.data()} as Registration;
  }
  async getRegistrationsByTournament(tournamentId: string): Promise<Registration[]> {
    const snapshot = await db.collection("tournament_registrations")
      ...
      .get();
    return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: doc.id, ...doc.data()} as Registration));
  }
  ...
  // Result operations
  async getResult(id: string | number): Promise<Result | undefined> {
    const doc = await db.collection("results").doc(id.toString()).get();
    return doc.exists ? {id: parseInt(doc.id), ...doc.data()} as Result : undefined;
  }
  async createResult(result: Partial<Result>): Promise<Result> {
    ...
    return {id: parseInt(newResult.id), ...newResult.data()} as Result;
  }
  async updateResult(id: string | number, resultData: Partial<Result>): Promise<Result | undefined> {
    const docRef = db.collection("results").doc(id.toString());
    await docRef.update(resultData);
    const updated = await docRef.get();
    return updated.exists ? {id, ...updated.data()} as Result : undefined;
  }
  async getResultsByTournament(tournamentId: string): Promise<Result[]> {
    const snapshot = await db.collection("results")
      .where("tournamentId", "==", tournamentId)
      .get();
    return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: parseInt(doc.id), ...doc.data()} as Result));
  }
  async getPendingResults(): Promise<Result[]> {
    const snapshot = await db.collection("results")
  ...
  async createTransaction(transaction: Partial<Transaction>): Promise<Transaction> {
    ...
    return {id: parseInt(newTransaction.id), ...newTransaction.data()} as Transaction;
  }
  async getTransactionsByUser(userId: string | number): Promise<Transaction[]> {
    const snapshot = await db.collection("transactions")
      .where("userId", "==", userId)
      .get();
    return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: parseInt(doc.id), ...doc.data()} as Transaction));
  }
  async getTransactionsByTournament(tournamentId: string): Promise<Transaction[]> {
    const snapshot = await db.collection("transactions")
      .where("tournamentId", "==", tournamentId)
      .get();
    return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: parseInt(doc.id), ...doc.data()} as Transaction));
  ...
  async getAnnouncement(id: string | number): Promise<Announcement | undefined> {
    const doc = await db.collection("announcements").doc(id.toString()).get();
    return doc.exists ? {id: parseInt(doc.id), ...doc.data()} as Announcement : undefined;
  }
  async createAnnouncement(announcement: Partial<Announcement>): Promise<Announcement> {
    ...
    return {id: parseInt(newAnnouncement.id), ...newAnnouncement.data()} as Announcement;
  }
  async updateAnnouncement(id: string | number, announcementData: Partial<Announcement>): Promise<Announcement | undefined> {
    const docRef = db.collection("announcements").doc(id.toString());
    await docRef.update(announcementData);
    const updated = await docRef.get();
    return updated.exists ? {id, ...updated.data()} as Announcement : undefined;
  }
  ...
  async getAllAnnouncements(): Promise<Announcement[]> {
    ...
    return snapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({id: parseInt(doc.id), ...doc.data()} as Announcement));
  }
  ...
  async syncUserFromAuth(uid: string): Promise<User | null> {
    try {
      if (!auth) {
  ...
  // KYC operations
  async createKycDocument(data: any): Promise<any> {
    try {
  ...
  async updateKycDocumentStatus(docId: string, updates: {
        status: string,
    ...
    try {
      console.log(`Updating KYC document status for doc ${docId}:`, updates);
      const statusUpper = updates.status.toUpperCase();
      console.log(`Normalized document status to uppercase: ${statusUpper}`);
  ...
  async updateUserKycStatus(userId: string, kycStatus: string): Promise<boolean> {
    try {
      console.log(`Updating KYC status for user ${userId} to ${kycStatus}`);
      const statusUpper = kycStatus.toUpperCase();
      console.log(`Normalized status to uppercase: ${statusUpper}`);
  ...
  async advancedUserSearch({q, status, role, kycStatus, from, to}: any): Promise<User[]> {
    let queryRef = db.collection("users") as any;
    if (status) queryRef = queryRef.where("status", "==", status);
    if (role) queryRef = queryRef.where("role", "==", role);
    if (kycStatus) queryRef = queryRef.where("kycStatus", "==", kycStatus);
    // Add more filters as needed
    const snapshot = await queryRef.get();
    let users = snapshot.docs.map((doc: any) => ({id: doc.id, ...doc.data()}));
    if (q) {
      users = users.filter((u: any) => (u.email?.includes(q) || u.username?.includes(q) || u.uid?.includes(q)));
    }
  ...
  // 5. Bulk Actions
  async bulkUserAction(action: string, userIds: string[], value?: any) {
    const results = [];
    for (const userId of userIds) {
      if (action === "delete") results.push(await this.deleteUser(userId));
      if (action === "activate") results.push(await this.updateUserStatus(userId, "active"));
      if (action === "deactivate") results.push(await this.updateUserStatus(userId, "inactive"));
      if (action === "ban") results.push(await this.updateUserStatus(userId, "banned"));
      if (action === "role" && value) results.push(await this.updateUserRole(userId, value));
    }
    return {success: true, results};
  }

  // 6. User Notes & Admin Comments
  async addUserNote(userId: string, note: string, adminEmail?: string) {
  ...
  async sendUserNotification(userId: string, {type, message, subject}: any) {
  ...
  async exportUsersCSV() {
    ...
    const rows = users.map((u: any) => Object.values(u).map((v) => `"${(v ?? "").toString().replace(/"/g, "\"\"")}"`).join(","));
    return [header, ...rows].join("\n");
  }
  async importUsersCSV(csv: string) {
    ...
    for (const line of lines) {
      ...
      if (user.id) await db.collection("users").doc(user.id).set(user, {merge: true});
    }
    return {success: true};
  }
  ...
  async getSupportTickets(filters?: {
    ...
    try {
      ...
      let query = this.db.collection("support_tickets") as any;
      ...
      const tickets = await Promise.all(
        snapshot.docs.map(async (doc: any) => {
          ...
          try {
            ...
            responses = responsesSnapshot.docs.map((responseDoc: any) => ({
              id: responseDoc.id,
              ...responseDoc.data(),
            }));
            ...
            console.warn(`Could not fetch responses for ticket ${doc.id}:`, (responsesError as Error).message);
            responses = [];
          }
  ...
  async getSupportTicketsStats(): Promise<{
      total: number;
    ...
    try {
      ...
      const avgResponseTime = tickets
        ...
        }, 0) / Math.max(tickets.filter((t: any) => t.status === "resolved").length, 1);
  ...
  async updateSupportTicketStatus(ticketId: string, status: "open" | "in-progress" | "resolved" | "closed"): Promise<SupportTicket> {
    try {
  ...
  async approveDepositRequest(depositId: string, adminId: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
      ...
      if (userDoc.exists) {
        ...
        await this.db.collection("transactions").add({
          ...
          status: "APPROVED",
          description: `Manual deposit approved - Ref: ${depositData.upiRefId}`,
          paymentMethod: "MANUAL",
          processed: true, // Mark as processed since admin is handling the balance update
        ...
        // Update any existing pending transaction to processed status
        const existingTransactionQuery = await this.db.collection("transactions")
          .where("depositRequestId", "==", depositId)
          .where("status", "==", "PENDING")
          .get();

        for (const doc of existingTransactionQuery.docs) {
  ...
  async rejectDepositRequest(depositId: string, adminId: string, reason: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
  ...
  async approveWithdrawalRequest(withdrawalId: string, adminId: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
      ...
      if (userDoc.exists) {
        ...
        await this.db.collection("transactions").add({
          ...
          status: "APPROVED",
          description: `Manual withdrawal approved to ${withdrawalData.upiId}`,
          paymentMethod: "MANUAL",
          processed: true, // Mark as processed since admin is handling the balance update
  ...
  async rejectWithdrawalRequest(withdrawalId: string, adminId: string, reason: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
  ...
  async getAdminUpiConfig(): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
  ...
  async updateAdminUpiConfig(config: any, adminId: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
  ...
  async getUserAllTransactions(userId: string): Promise<any> {
    try {
      if (!this.db) {
        throw new Error("Database not initialized");
      }
      ...
      const [
        ...
        this.db.collection("tournament_registrations").where("userId", "==", userId).get(),
      ]);
      ...
      walletTransactions.docs.forEach((doc) => {
        ...
        if (data.type === "deposit") totalDeposits += transaction.amount;
        if (data.type === "withdrawal") totalWithdrawals += transaction.amount;
        if (data.type === "winning" || data.type === "prize") totalWinnings += transaction.amount;
      });
      ...
      // Process tournament registrations for winnings
      tournamentRegistrations.docs.forEach((doc) => {
        const data = doc.data();
        if (data.reward && data.reward > 0 && data.resultVerified) {
          const transaction = {
            id: doc.id,
            type: "winning",
            amount: data.reward || 0,
            currency: "INR",
            status: data.prizeDistributed ? "completed" : "pending",
  ...
  private formatTimestamp(timestamp: any): Date | null {
  ...
  async getKycDocument(docId: string): Promise<any | null> {
    try {
  ...
  async creditUserWallet(userId: string | number, amount: number, currency: string, description: string) {
