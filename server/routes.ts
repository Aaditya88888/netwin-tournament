...
// import { auth } from "./lib/firebase.js";  // Commented out for now, using direct JS import in index.js
import {
  User as SchemaUser,
  Tournament,
...
// Extend the base Request type to include Firebase Auth user info
...
// Define an interface that matches what storage.getAllUsers returns
...
import {appCheckMiddleware, addAppCheckHeaders} from "./middleware/appcheck.js";
...
export function registerRoutes(app: Express): void {
  ...
  const registerRoute = (method: "get" | "post" | "put" | "patch" | "delete", path: string, ...handlers: any[]) => {
  ...
  const calculatePoints = (kills: number, position: number | null): number => {
    let points = 0;
  ...
  registerRoute("get", "/debug/routes", (req: Request, res: Response) => {
    const routes: any[] = [];

    // Extract routes from Express app
    function extractRoutes(app: any) {
  ...
  registerRoute("get", "/debug/admin-kyc", asyncHandler(async (req, res) => {
    ...
    try {
      // Get all users
      const allUsers = (await storage.getAllUsers()) as FirestoreUser[];
      ...
      // Sort users
      if (sortBy && sortOrder) {
        kycUsers.sort((a: any, b: any) => {
          let aValue: any = a[sortBy as keyof FirestoreUser];
          let bValue: any = b[sortBy as keyof FirestoreUser];
  ...
  registerRoute("post", "/users/:userId/kyc", appCheckMiddleware, authenticateToken, asyncHandler(async (req: AuthRequest, res) => {
  ...
  const adminUserStatsHandler = asyncHandler(async (req, res) => {
    const allUsers = (await storage.getAllUsers()) as FirestoreUser[];
    const pendingKyc = (await storage.getPendingKycUsers()) as FirestoreUser[];
    // Helper function to safely count users by status
    const countByStatus = (users: FirestoreUser[], status: string) => users.filter((u) => u.status === status).length;
    // Helper function to safely count users by KYC status
    const countByKycStatus = (users: FirestoreUser[], status: string) => users.filter((u) => u.kycStatus === status).length;
  ...
  registerRoute("get", "/users/search", authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    const {page = 1, limit = 10, search = "", status = "", kycStatus = ""} = req.query; const allUsers = await storage.getAllUsers();

    let filteredUsers: FirestoreUser[] = allUsers.map((user) => ({
      ...user,
  ...
  // Delete a user by ID
  registerRoute("delete", "/users/:userId", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req: AuthRequest, res: Response) => {
    const {userId} = req.params;
  ...
  registerRoute("get", "/users/:userId/kyc", appCheckMiddleware, authenticateToken, asyncHandler(async (req: Request, res: Response) => {
    const {userId} = req.params;
    const authUserId = (req as AuthRequest).user?.uid;

    // Verify the authenticated user is either an admin or the owner of this KYC document
    if (!authUserId || (authUserId !== userId && (req as AuthRequest).user?.role !== "admin")) {
  ...
  registerRoute("get", "/admin/kyc", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const [allUsers, allKycDocuments] = await Promise.all([
        storage.getAllUsers() as Promise<FirestoreUser[]>,
      ...
      if (status && status !== "all") {
        ...
        kycUsers = kycUsers.filter((user: any) => {
          const userKycDocs = kycDocumentsByUser.get(user.id || user.uid) || [];
          const latestKycDoc = userKycDocs.sort((a: any, b: any) => {
  ...
  registerRoute("post", "/users/:id/add-bonus", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req: AuthRequest, res) => {
    const {id} = req.params;
  ...
  registerRoute("get", "/tournaments", asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({message: (error as any).message || "Failed to fetch tournaments", error: (error as any).message});
    }
  }));
  ...
  registerRoute("get", "/tournaments/:id/results-management", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    try {
      ...
      const players = registrationsSnapshot.docs.map((doc: any) => {
      ...
      res.json({
        ...
        playersWithResults: players.filter((p: any) => p.resultSubmitted).length,
      });
    } catch (error) {
  ...
  registerRoute("get", "/debug/wallet-data", asyncHandler(async (req, res) => {
    try {
      ...
      const deposits = depositsSnapshot.docs.map((doc: any) => ({
        id: doc.id,
      ...
      const withdrawals = withdrawalsSnapshot.docs.map((doc: any) => ({
        id: doc.id,
        status: doc.data().status,
        amount: doc.data().amount,
        createdAt: doc.data().createdAt,
      ...
      res.json({
        ...
        depositStatuses: deposits.map((d: any) => d.status),
        withdrawalStatuses: withdrawals.map((w: any) => w.status),
        pendingDeposits: deposits.filter((d: any) => d.status === "PENDING"),
        approvedDeposits: deposits.filter((d: any) => d.status === "APPROVED"),
        rejectedDeposits: deposits.filter((d: any) => d.status === "REJECTED"),
        deposits: deposits,
  ...
  registerRoute("get", "/wallet/deposits", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    try {
      const snapshot = await db.collection("pending_deposits")
        ...
        .get();
      const deposits = snapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
  ...
  registerRoute("get", "/wallet/withdrawals", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    try {
      const snapshot = await db.collection("pending_withdrawals")
        .orderBy("createdAt", "desc")
        .get();
      const withdrawals = snapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
        createdAt: doc.data().createdAt?.toDate?.() || new Date(doc.data().createdAt),
        updatedAt: doc.data().updatedAt?.toDate?.() || (doc.data().updatedAt ? new Date(doc.data().updatedAt) : null),
  ...
  registerRoute("get", "/wallet/transactions", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    try {
      ...
      // Apply filters
      if (type && type !== "all") {
        allTransactions = allTransactions.filter((t: any) => t.type === type);
      }

      if (status && status !== "all") {
        allTransactions = allTransactions.filter((t: any) => t.status?.toLowerCase() === (status as string).toLowerCase());
      }

      if (userId) {
        allTransactions = allTransactions.filter((t: any) => t.userId === userId);
      }
  ...
  registerRoute("post", "/wallet/config", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    try {
      ...
      for (const [currency, config] of Object.entries(configData) as any[]) {
  ...
  // Approve deposit
  registerRoute("post", "/wallet/deposits/:id/approve", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req: AuthRequest, res) => {
    const depositId = req.params.id;
    if (!depositId) return res.status(400).json({message: "Missing deposit ID"});
    ...
    try {
      ...
      if (!userId) return res.status(400).json({message: "Deposit missing userId"});
      ...
      const newBalance = (user.balance || 0) + (deposit.amount || 0);
  ...
  registerRoute("post", "/wallet/withdrawals/:id/approve", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    try {
      ...
      if (!withdrawalSnap.exists) {
        ...
        if (!withdrawalSnap.exists) return res.status(404).json({message: "Withdrawal not found"});
      }
      ...
      if (!userId) return res.status(400).json({message: "Withdrawal missing userId"});
      ...
      await withdrawalRef.update({status: "APPROVED", verifiedAt: new Date(), verifiedBy: (req as AuthRequest).user?.email || "admin", updatedAt: new Date()});
      ...
      if (!transactionsSnap.empty) {
        ...
        await transactionDoc.ref.update({
          ...
          updatedAt: new Date(),
          verifiedAt: new Date(),
          verifiedBy: (req as AuthRequest).user?.email || "admin",
        });
        console.log("Updated transaction in transactions collection:", transactionDoc.id);
      } else {
    ...
    try {
      ...
      const statusInfo = {
        tournament: {
          ...
          maxTeams: (tournament as any).maxPlayers || 100,
        ...
        players: registrations.map((reg: any) => ({
  ...
  registerRoute("get", "/tournaments/:id/prize-distribution", appCheckMiddleware, authenticateToken, requireAdminOrModerator, asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const commissionPercentage = (tournament as any).companyCommissionPercentage || 10;
      ...
      const actualPrizePool = (tournament as any).prizePool || (totalEntryFees - companyCommission);
      ...
      const firstPrizePercentage = (tournament as any).firstPrizePercentage || 40;
      const perKillPercentage = (tournament as any).perKillRewardPercentage || 60;
      ...
      const totalKills = registrations.reduce((sum: any, reg: any) => sum + (reg.kills || 0), 0);
      ...
      const prizeData = {
        ...
        players,
        canDistribute: tournament.status === "completed" && !!(tournament as any).prizesDistributed !== true,
        totalPlayers: players.length,
      };
  ...
  registerRoute("get", "/tournaments/:id/transactions", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const allTransactions = transactionsSnapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
  ...
  registerRoute("post", "/tournament-management/send-tournament-notification/:id", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const registrations = registrationsSnapshot.docs.map((doc: any) => doc.data());
  ...
  registerRoute("post", "/tournaments/:id/distribute-prizes", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if ((tournament as any).prizesDistributed) {
      ...
      const verifiedRegistrations = registrations.filter((reg: any) =>
        reg.resultVerified && ((reg.position === 1) || (reg.kills > 0))
      );
      ...
      const totalKills = verifiedRegistrations.reduce((sum: any, reg: any) => sum + (reg.kills || 0), 0);
  ...
  registerRoute("get", "/admin/dashboard/stats", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    try {
      ...
      const deposits = depositsSnapshot.docs.map((doc: any) => doc.data());
      const withdrawals = withdrawalsSnapshot.docs.map((doc: any) => doc.data());
  ...
  registerRoute("get", "/support-tickets", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    try {
      ...
      query = query.orderBy(sortBy as string, sortOrderValue);
      ...
      console.log("📧 [Backend] Ticket statuses:", tickets.map((t: any) => ({id: t.ticketId, status: t.status})));
  ...
  registerRoute("get", "/support-tickets/stats", appCheckMiddleware, authenticateToken, requirePermission("admin"), asyncHandler(async (req, res) => {
    try {
      ...
      const stats = {
        ...
        medium_priority: tickets.filter((t: any) => t.priority === "medium").length,
        low_priority: tickets.filter((t: any) => t.priority === "low").length,
        by_category: {
          tournament: tickets.filter((t: any) => t.category === "tournament").length,
  ...
  // OTP Routes
  registerRoute("post", "/auth/send-otp", asyncHandler(async (req: Request, res: Response) => {
    try {
  ...
  registerRoute("post", "/auth/verify-otp", asyncHandler(async (req: Request, res: Response) => {
    try {
