      ...
      <Dialog open={!!selectedTransaction} onOpenChange={() => setSelectedTransaction(null)}>
        <DialogContent className="max-w-4xl w-[95vw] max-h-[95vh] overflow-y-auto">
          ...
          {selectedTransaction && (            <div className="space-y-6">
              ...
              {/* Status information for non-pending transactions */}
              {(selectedTransaction.status === 'APPROVED' || selectedTransaction.status === 'REJECTED') && (
                <div className={`rounded-lg p-4 border ${
                  ...
                  <h3 className={`text-sm font-semibold mb-2 ${
                    ...
                    Transaction {selectedTransaction.status === 'APPROVED' ? 'Approved' : 'Rejected'}
                  </h3>
                  <p className={`text-sm ${
                    selectedTransaction.status === 'APPROVED' 
                    ...
                    {selectedTransaction.status === 'APPROVED' 
                      ? 'This transaction has been approved and processed successfully.' 
                    ...
                    (('processedAt' in selectedTransaction && selectedTransaction.processedAt) || 
                     ('verifiedAt' in selectedTransaction && selectedTransaction.verifiedAt)) && (
                      <div className="mt-3 pt-3 border-t border-green-200 dark:border-green-700">
                        <p className="text-sm font-medium text-green-700 dark:text-green-300">Processing Information:</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2 text-sm">
                          <div>
                            <span className="text-green-600 dark:text-green-400">Processed At:</span>
                          ...
                          {(('processedBy' in selectedTransaction && selectedTransaction.processedBy) || 
                            ('verifiedBy' in selectedTransaction && selectedTransaction.verifiedBy)) && (
                            <div>
                              <span className="text-green-600 dark:text-green-400">Processed By:</span>
                              <p className="text-green-800 dark:text-green-200">
                                {('processedBy' in selectedTransaction && selectedTransaction.processedBy) || 
                                 ('verifiedBy' in selectedTransaction && selectedTransaction.verifiedBy)}
                              </p>
